...
public class SubstepsLexer extends LexerBase {
  ...
  public SubstepsLexer(SubstepsKeywordProvider provider) {
  ...
  private void updateLanguage(String language) {
    ...
    myKeywords = new ArrayList<String>(myKeywordProvider.getAllKeywords());
  ...
  public void advance() {
    ...
    if (myState != STATE_INSIDE_PYSTRING && Character.isWhitespace(c)) {
      ...
      myCurrentToken = SubstepsTokenTypes.PIPE;
      myPosition++;
      myState = STATE_TABLE;
      ...
      if (c == '>') {
        ...
        myPosition++;
        myCurrentToken = SubstepsTokenTypes.STEP_PARAMETER_BRACE;
      } else {
        advanceToParameterEnd(PYSTRING_MARKER);
        myCurrentToken = SubstepsTokenTypes.STEP_PARAMETER_TEXT;
      }
    } else if (myState == STATE_INSIDE_PYSTRING) {
      if (isStringAtPosition(PYSTRING_MARKER)) {
        myPosition += 3 /* marker length */;
        myCurrentToken = SubstepsTokenTypes.PYSTRING;
        ...
        if (myBuffer.charAt(myPosition) == '<') {
          if (isStepParameter(PYSTRING_MARKER)) {
            ...
            myCurrentToken = SubstepsTokenTypes.PYSTRING_TEXT;
          }
        } else {
          advanceToParameterOrSymbol(PYSTRING_MARKER, STATE_INSIDE_PYSTRING, false);
          myCurrentToken = SubstepsTokenTypes.PYSTRING_TEXT;
      ...
      myCurrentToken = SubstepsTokenTypes.TABLE_CELL;
      ...
      myCurrentToken = SubstepsTokenTypes.COMMENT;
      advanceToEOL();
      ...
      myCurrentToken = SubstepsTokenTypes.COLON;
      myPosition++;
    }
    else if (c == '@') {
      myCurrentToken = SubstepsTokenTypes.TAG;
      myPosition++;
      ...
      if (myState == STATE_DEFAULT) {
        for (String keyword : myKeywords) {
          ...
          if (isStringAtPosition(keyword)) {
            if (myKeywordProvider.isSpaceAfterKeyword(keyword) &&
            ...
            myPosition += length;
            if (myCurrentToken == SubstepsTokenTypes.STEP_KEYWORD) {
      ...
      if (myState == STATE_PARAMETER_INSIDE_STEP) {
        ...
        if (myPosition < myEndOffset && myBuffer.charAt(myPosition) == '<' && isStepParameter("\n")) {
          ...
          myCurrentToken = SubstepsTokenTypes.TEXT;
